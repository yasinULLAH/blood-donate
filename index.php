...
function validate_input($input, $type, $options = [])
{
    ...
    switch ($type) {
        ...
        case 'status':
            $valid_statuses = ['pending', 'fulfilled', 'closed', 'upcoming', 'completed', 'cancelled', 'available', 'used', 'expired', 'quarantined'];
...
<?php
            ...
            case 'add_blood_bag':
                ...
                if ($custom_expiry_date && new DateTime($custom_expiry_date) < new DateTime($collection_date)) {
                    set_flash_message('danger', 'Expiry date cannot be before the collection date.');
                    redirect('admin_blood_bank');
                }
            ...
            case 'update_bag_status':
                ...
                if (!$inventory_id) {
                    $errors[] = "Bag ID is invalid.";
                }
                if (!$status) {
                    $errors[] = "Status value is invalid ('" . sanitize($_POST['status'] ?? 'NULL') . "').";
                }
                if (!$expiry_date) {
                    $errors[] = "Expiry Date is invalid or not in YYYY-MM-DD format ('" . sanitize($_POST['expiry_date'] ?? 'NULL') . "').";
                }

                if (!empty($errors)) {
                    $error_message = "Update failed: " . implode(" ", $errors);
                    set_flash_message('danger', $error_message);
                    redirect('admin_blood_bank');
                }

                $stmt = $db->prepare("UPDATE blood_inventory SET status = ?, expiry_date = ? WHERE id = ?");
                $stmt->execute([$status, $expiry_date, $inventory_id]);
            ...
            case 'change_password':
                ...
                $email = validate_input($_POST['email'], 'email');
                $contact = validate_input($_POST['contact_number'], 'tel');
                $blood_group = validate_input($_POST['blood_group'], 'blood_group') ?: 'N/A';
                $city = validate_input($_POST['city'], 'string', ['min_len' => 2]);
                ...
                $stmt = $db->prepare("
                    INSERT INTO profiles (user_id, blood_group, city, is_available, last_donation_date, updated_at)
                ...
                ");
                $stmt->execute([$user_id, $blood_group, $city, $is_available, $last_donation_date]);
...
<head>
    ...
    <style>
        ...
        .hero-section {
            background: linear-gradient(rgba(255, 255, 255, 0.9), rgba(255, 255, 255, 0.9)), url('bg.jpg') no-repeat center center;
                    ...
                    <div class="list-group list-group-flush mx-3 mt-4">
                        ...
                        <?php if (get_current_user_role() === 'admin'): ?>
                            <div class="pt-3">
                                ...
                                <a href="finance.php" target="_blank" class="list-group-item list-group-item-action py-2 ripple">
                                                        ...
                                                        <tr class="<?= $row_class ?>">
                                                            ...
                                                            <td>
                                                                <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#updateStatusModal"
                                                                    data-bag-id="<?= sanitize($item['bag_id']) ?>"
                                                                    data-inventory-id="<?= sanitize($item['id']) ?>"
                                                                    data-current-status="<?= sanitize($item['status']) ?>"
                                                                    data-expiry-date="<?= sanitize($item['expiry_date']) ?>">
